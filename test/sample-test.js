...
describe("RandomMinter", function () {
  ...
  it("Should transfer random nft from owner wallet to smart contract caller", async function () {
    ...
    for(let i = 1; i <= 672; i++ ){
      const tx = await metashipNftContract.connect(acc1).transferFrom(acc1.address, acc4.address, ethers.utils.formatUnits(i.toString(), 0));
      await tx.wait();
    }
    ...
    tx = await randomMetashipNftContract.connect(acc2).mintRandom("test mint", {value: ethers.utils.parseEther("0.2")});
    ...
    tx = await randomMetashipNftContract.connect(acc3).mintRandom("test mint acc3", {value: ethers.utils.parseEther("0.2")});
    ...
    let mintTxs = [];
    for(let i = 0; i < 98; i++) {
      const transaction = await randomMetashipNftContract.connect(acc2).mintRandom("test mint", {value: ethers.utils.parseEther("0.2")});
    ...
    await expect(randomMetashipNftContract.connect(acc3).mintRandom("test mint acc3", {value: ethers.utils.parseEther("0.2")})).to.be.revertedWith('Not enough ether');
